...
- **Pemasangan dikunci per tool**, bukan satu kunci global, dan panel Tools melacak status sibuk per tool. yt-dlp bisa dipasang selagi unduhan FFmpeg yang jauh lebih besar berjalan, tetapi tool yang sama tidak bisa dipasang dua kali bersamaan. Penyimpanan `tool-updates.json` diserialkan dan selalu menulis state terkini, supaya dua pemasangan yang selesai hampir bersamaan tidak saling menimpa. Batasan satu-per-satu yang lama juga ditemukan dari uji coba pengguna.
